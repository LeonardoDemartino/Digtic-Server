...
public class EntityNPCRock extends Entity{
	//sounds
	static int walkSound=-1,hitSound=-1,terrainCreateSound=-1;
	
	EntityNPCRock(Server server, int tid, float x, float y, float z, GameLogic game){
		super(server);
		//TYPE ID FOR NETWORK INFORMATION
		type=Entity.TYPE_EntityNPCRock;
		///////////////
		//textureIndex=tid;
		setPosition(x, y, z);
		setRotation(0,0,0);
		...
		/*if(walkSound==-1 || hitSound==-1 || terrainCreateSound==-1){
			...
			hitSound=this.game.soundManager.load(context, R.raw.shot_rock, 1);
