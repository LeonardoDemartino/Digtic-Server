...
public class EntityNPCWater extends Entity{
	//sounds
	static int walkSound=-1,hitSound=-1,terrainCreateSound=-1;
		
	EntityNPCWater(Server server, int tid, float x, float y, float z, GameLogic game){
		super(server);
		//TYPE ID FOR NETWORK INFORMATION
		type=Entity.TYPE_EntityNPCWater;
		///////////////
		//textureIndex=tid;
		setPosition(x, y, z);
		setRotation(0,0,0);
		...
		/*if(walkSound==-1 || hitSound==-1 || terrainCreateSound==-1){
			...
			hitSound=this.game.soundManager.load(context, R.raw.shot_water, 1);
	...
	void update(){
		...
		if(toAttack>=0 && toAttack<game.objects.size()){
			...
			if(dirMod>minDistance){
				...
				if(rnd<10)addAction(EventsHandler.ACTION_JUMP);
			}
		}
		else{
			if(rnd<200)
